<template>
	...
	<div class="row modal-body" ref="formCadastro" id="formRegra"><!-- BODY -->
		...
		<div class="col-11 caixa row">
			...
			<div class="col-2" v-if="form.situacaoTributaria == '99'">
				<label>Base %</label>
				<input type="text" class="form-control" v-money="money" v-model="form.Base">
			</div>

			<div class="col-4" >
				<label>Cód. de enquadramento</label>
				<select class="form-control" v-model="form.CodEnquadramento">
					<template v-for="situacao in situacaoTributaria">
						<option v-for="item in situacao.codEnquadramento" v-if="form.situacaoTributaria == situacao.id" :value="item">{{item}}</option>
					</template>
				</select>
			</div>
